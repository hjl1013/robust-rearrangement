...
from src.dataset.dataset import ImageDataset, StateDataset, ImageReverseDataset, StateReverseDataset
...
def main(cfg: DictConfig):
    ...
    dataset: Union[ImageReverseDataset, StateReverseDataset]

    # Use reverse datasets for reverse diffusion policy
    # Reverse datasets take the same arguments as regular datasets but predict past actions instead of future actions
    if cfg.observation_type == "image":
        dataset = ImageReverseDataset(
    ...
    # Load latent model for dense reward if specified
    latent_model = None
    if cfg.rollout.get("dense_reward", False):
        latent_model_path = cfg.rollout.get("latent_model_path", None)
        if latent_model_path is None:
            raise ValueError(
                "latent_model_path must be specified in config when dense_reward=True. "
                "This should point to a pre-trained forward policy checkpoint."
            )
        print(f"Loading latent model from {latent_model_path}")
        latent_checkpoint = torch.load(latent_model_path, map_location=device)
        
        # Get config from checkpoint
        if "config" in latent_checkpoint:
            latent_cfg = OmegaConf.create(latent_checkpoint["config"])
        else:
            raise ValueError("Config not found in latent model checkpoint.")
        
        # Create latent model actor
        latent_model: Actor = get_actor(cfg=latent_cfg, device=device)
        
        # Set normalizer if available in checkpoint (needed for latent computation)
        if "normalizer" in latent_checkpoint.get("config", {}):
            # Normalizer should be in the model state dict
            pass
        
        # Load weights (this will also load normalizer if present)
        if "model_state_dict" in latent_checkpoint:
            latent_model.load_state_dict(latent_checkpoint["model_state_dict"])
        else:
            latent_model.load_state_dict(latent_checkpoint)
    ...
    for epoch_idx in tglobal:
        ...
        if (
            ...
            if (
                ...
                if env is None:
                    from src.gym import get_rl_reverse_env
                    env = get_rl_reverse_env(
                        ...
                        headless=True,
                        verbose=True,
                        dense_reward=cfg.rollout.get("dense_reward", False),
                        dense_reward_threshold=cfg.rollout.get("dense_reward_threshold", 0.1),
                        latent_model=latent_model if cfg.rollout.get("dense_reward", False) else None,
                    )
